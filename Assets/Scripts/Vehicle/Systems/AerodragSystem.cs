...
{
    /// <summary>
    /// Aerodynamic drag system that applies physically accurate aerodynamic drag resistance.
    /// Formula: F_drag = -0.5 * ρ * Cd * A * v² * direction
    /// Where:
    /// - ρ (airDensity) - air density (constant: 1.225 kg/m³)
    /// - Cd (dragCoefficient) - drag coefficient from ChassisSpec
    /// - A (frontArea) - frontal area from ChassisSpec
    /// - v - velocity magnitude
    /// - direction - normalized velocity vector
    /// </summary>
    public sealed class AerodragSystem : IVehicleModule
    {
        private readonly float _airDensity;
        private readonly float _minSpeed;

        /// <summary>
        /// Initializes a new instance of the AerodragSystem.
        /// </summary>
        ...
        public AerodragSystem(float airDensity, float minSpeed)
        {
        ...
        /// </summary>
        /// <param name="input">Current vehicle input (not used in this system).</param>
        /// <param name="state">Current vehicle state (not used in this system).</param>
        /// <param name="ctx">Vehicle context containing rigidbody and chassis specifications.</param>
        public void Tick(in VehicleInput input, ref VehicleState state, in VehicleContext ctx)
        {
            ...
            // Get aerodynamic parameters from ChassisSpec
            if (ctx.chassisSpec == null) return;

            float frontArea = ctx.chassisSpec.frontArea;
            float dragCoefficient = ctx.chassisSpec.dragCoefficient;
